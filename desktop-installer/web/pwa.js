(function () {
  ...
  const SEARCH_MODE_KEY = "veille-immo-search-mode";
  const DEFAULT_SEARCH_MODE = "house";
  const SEARCH_MODES = {
    house: {
      key: "house",
      propertyType: "maison",
    ...
    land: {
      ...
      labelSingular: "terrain à bâtir",
      resultsFile: "results-terrain.json",
      maxPrice: 100000
    }
  };
  ...
  let currentSearchMode = storedSearchMode();
  ...
  function storedSearchMode() {
    try {
      ...
      return SEARCH_MODES[stored] ? stored : DEFAULT_SEARCH_MODE;
    } catch (error) {
  ...
  function activeSearchProfile() {
    return SEARCH_MODES[currentSearchMode] || SEARCH_MODES[DEFAULT_SEARCH_MODE];
  }

  function modeStorageKey(baseKey) {
    return baseKey + "-" + activeSearchProfile().key;
  }

  function configuredSearchMode(config, mode) {
    const key = mode || currentSearchMode;
    const fallback = SEARCH_MODES[key] || SEARCH_MODES[DEFAULT_SEARCH_MODE];
    const configured = config && config.searchModes && config.searchModes[key] ? config.searchModes[key] : {};
  ...
  function injectControls() {
    ...
    style.textContent = [
      ...
      ".search-mode-switch{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin:12px 0 14px;padding:7px;background:#e8eef2;border:1px solid #cbd5dc;border-radius:10px;width:max-content;max-width:100%;box-sizing:border-box}",
      ".search-mode-switch-label{font:700 13px Arial,sans-serif;color:#33424d;padding:0 5px}",
      ".search-mode-buttons{display:inline-flex;gap:4px;flex-wrap:wrap}",
      ".search-mode-button{border:1px solid transparent;background:transparent;color:#253540;border-radius:7px;padding:9px 14px;font:700 14px Arial,sans-serif;cursor:pointer}",
      ".search-mode-button[aria-pressed='true']{background:#0b5c86;color:#fff;box-shadow:0 2px 7px rgba(11,92,134,.24)}",
      ".search-mode-button:focus-visible{outline:3px solid rgba(11,92,134,.28);outline-offset:2px}",
      ...
      "@media(max-width:760px){.search-mode-switch{width:100%}.search-mode-switch-label{width:100%}.search-mode-buttons{display:grid;grid-template-columns:1fr 1fr;width:100%}.price-filter-panel{grid-template-columns:1fr}.price-filter-panel>div:first-child{display:block;white-space:normal}.price-filter-count{display:block;margin:4px 0 0}.price-filter-inputs{justify-content:flex-start;flex-wrap:wrap}.price-filter-inputs input{width:min(140px,100%)}.filter-source-counts{white-space:normal}.location-distance-filter{grid-template-columns:1fr;align-items:start;min-width:100%}.location-distance-value{justify-self:start}}",
      "@media(max-width:680px){html,body{max-width:100%;overflow-x:hidden}main{width:100%;max-width:100%;box-sizing:border-box;padding:12px;padding-left:max(12px,env(safe-area-inset-left));padding-right:max(12px,env(safe-area-inset-right));padding-bottom:calc(82px + env(safe-area-inset-bottom))}h1{font-size:23px;line-height:1.2;overflow-wrap:anywhere}h2{font-size:19px;line-height:1.25}.meta,.note,.listing-title,.contact,.facts,.links{overflow-wrap:anywhere;word-break:break-word}.cards{grid-template-columns:minmax(0,1fr);width:100%}.listing-card,.listing-body,.photo-strip{min-width:0;max-width:100%;box-sizing:border-box}.photo-strip img{width:min(76vw,280px);height:auto;aspect-ratio:4/3}.search-mode-button{padding:10px 8px;min-width:0}.pwa-controls{left:max(12px,env(safe-area-inset-left));right:max(12px,env(safe-area-inset-right));bottom:max(12px,env(safe-area-inset-bottom))}.pwa-controls button{flex:1;min-width:0}.pwa-status{left:12px;right:12px;bottom:calc(68px + env(safe-area-inset-bottom));max-width:none}#map{height:min(58vh,420px);min-height:300px;width:100%}.map-tools,.map-legend{width:100%}.map-toggle{max-width:100%;box-sizing:border-box}.responsive-data-table{display:block;width:100%;max-width:100%;background:transparent;border-collapse:separate}.responsive-data-table thead{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.responsive-data-table tbody{display:grid;width:100%;gap:10px}.responsive-data-table tr{display:block;width:100%;box-sizing:border-box;background:#fff;border:1px solid #d8dee3;border-radius:8px;overflow:hidden}.responsive-data-table td{display:grid;grid-template-columns:minmax(92px,36%) minmax(0,1fr);gap:10px;width:100%;box-sizing:border-box;border:0;border-bottom:1px solid #edf0f2;padding:8px 10px;overflow-wrap:anywhere;word-break:break-word}.responsive-data-table td:last-child{border-bottom:0}.responsive-data-table td:empty{display:none}.responsive-data-table td::before{content:attr(data-label);font-weight:700;color:#4d5963}.responsive-data-table a,.responsive-data-table button{max-width:100%;overflow-wrap:anywhere;word-break:break-word}}"
    ].join("");
    ...
    injectSearchModeSwitch();
  }

  function updateSearchModeSwitch() {
    document.querySelectorAll("[data-search-mode]").forEach(function (button) {
      const active = button.dataset.searchMode === currentSearchMode;
      button.setAttribute("aria-pressed", active ? "true" : "false");
    });
  }

  function injectSearchModeSwitch() {
    let switcher = document.getElementById("searchModeSwitch");
    if (!switcher) {
      switcher = document.createElement("section");
      switcher.id = "searchModeSwitch";
      switcher.className = "search-mode-switch";
      switcher.setAttribute("aria-label", "Type de bien recherché");
      switcher.innerHTML = [
        "<span class='search-mode-switch-label'>Recherche</span>",
        "<div class='search-mode-buttons'>",
        "<button type='button' class='search-mode-button' data-search-mode='house'>Maisons</button>",
        "<button type='button' class='search-mode-button' data-search-mode='land'>Terrains à bâtir</button>",
        "</div>"
      ].join("");
      ...
      if (heading && heading.parentNode) {
        ...
        (document.querySelector("main") || document.body).prepend(switcher);
      }
    }
    updateSearchModeSwitch();
  }

  async function setSearchMode(mode) {
    if (!SEARCH_MODES[mode] || mode === currentSearchMode) {
      return;
    }
    ...
    currentPriceConfigMax = null;
    latestPayload = null;
    openingBaselineCache = null;
    newListingIds = new Set();
    updateSearchModeSwitch();
    showStatus("Chargement de la recherche « " + activeSearchProfile().label + " »…");
    try {
      await refreshReportData(false);
      showStatus("Carte et liste mises à jour : " + activeSearchProfile().label + ".");
    } catch (error) {
      currentSearchMode = previousMode;
      try {
        localStorage.setItem(SEARCH_MODE_KEY, previousMode);
      } catch (storageError) {
      }
      updateSearchModeSwitch();
      showStatus("La recherche demandée est momentanément indisponible.");
    }
  }
  ...
  function seenIdsFromStorage() {
    try {
      return new Set(JSON.parse(localStorage.getItem(modeStorageKey(STORAGE_KEY)) || "[]"));
    } catch (error) {
      return new Set();
    }
  }
  ...
  function saveSeenIds(ids) {
    localStorage.setItem(modeStorageKey(STORAGE_KEY), JSON.stringify(Array.from(ids)));
    localStorage.setItem(modeStorageKey(INIT_KEY), "1");
  }
  ...
  function previousLaunchIdsFromStorage() {
    const explicit = storedIdSet(modeStorageKey(LAST_LAUNCH_KEY));
    if (explicit.available) {
      return { available: explicit.ids.size > 0, ids: explicit.ids, source: "previous-launch" };
    }
    const migrated = storedIdSet(modeStorageKey(STORAGE_KEY));
    return {
      available: localStorage.getItem(modeStorageKey(INIT_KEY)) === "1" && migrated.ids.size > 0,
      ids: migrated.ids,
  ...
  function openingBaselineFromSession() {
    try {
      const raw = sessionStorage.getItem(modeStorageKey(OPENING_BASELINE_KEY));
      if (!raw) {
        return null;
      }
  ...
  function saveOpeningBaselineToSession(baseline) {
    try {
      sessionStorage.setItem(modeStorageKey(OPENING_BASELINE_KEY), JSON.stringify({
  ...
  function saveCurrentLaunchSnapshot(payload) {
    ...
    try {
      localStorage.setItem(modeStorageKey(LAST_LAUNCH_KEY), JSON.stringify(Array.from(ids)));
    } catch (error) {
    }
  }
  ...
  function configuredPriceMax(config) {
    const mode = configuredSearchMode(config);
    return Number(mode.maxPrice || (config && config.maxPrice) || DEFAULT_MAX_PRICE);
  }
  ...
  function priceBounds(payload, config) {
    ...
    const max = configuredPriceMax(config);
  ...
  function storedPriceMax(bounds, config) {
    ...
    try {
      const storedRaw = localStorage.getItem(modeStorageKey(PRICE_FILTER_KEY));
      const storedConfigMax = Number(localStorage.getItem(modeStorageKey(PRICE_FILTER_CONFIG_KEY)) || 0);
  ...
  function saveStoredPriceMax(value, config) {
    try {
      localStorage.setItem(modeStorageKey(PRICE_FILTER_KEY), String(value));
      localStorage.setItem(modeStorageKey(PRICE_FILTER_CONFIG_KEY), String(configuredPriceMax(config || latestConfig)));
    } catch (error) {
    }
  }
  ...
  function updateReportHeader(payload, config) {
    ...
    const profile = configuredSearchMode(config || latestConfig);
    meta.textContent = profile.label + " à vendre jusqu'à " + formatPrice(configuredPriceMax(config || latestConfig)) + (dateText ? " - données du " + dateText : "") + " - " + listings.length + " annonce(s), " + counts + ".";
    const heading = document.querySelector("main h1") || document.querySelector("h1");
    if (heading) {
      heading.textContent = "Veille immobilière quotidienne — " + profile.label;
    }
  }
  ...
  function secondHandSearchUrl(location, maxPrice) {
    const category = currentSearchMode === "land" ? "terrains-terrains-a-batir" : "maisons-a-vendre";
    return "https://www.2ememain.be/l/immo/" + category + "/q/" + encodeURIComponent(slugForPath(location.name)) + "/?priceTo=" + encodeURIComponent(maxPrice);
  }

  function facebookMarketplaceSearchUrl(location, maxPrice) {
    return "https://www.facebook.com/marketplace/search/?query=" + encodeURIComponent(activeSearchProfile().labelSingular + " a vendre " + location.name + " " + maxPrice);
  }

  function privateWebSearchUrl(location, maxPrice) {
    return "https://www.bing.com/search?q=" + encodeURIComponent(activeSearchProfile().labelSingular + " a vendre " + location.name + " " + maxPrice + " particulier sans agence");
  }
    ...
    scope.querySelectorAll("table").forEach(function (table) {
      ...
      table.querySelectorAll("tbody tr").forEach(function (row) {
        Array.from(row.children).forEach(function (cell, index) {
          if (cell.tagName === "TD") {
            cell.setAttribute("data-label", labels[index] || "Information");
          }
        });
      });
    });
  ...
  function agencyContactBlocks() {
    const main = document.querySelector("main") || document.body;
    return Array.from(main.querySelectorAll("h2")).filter(function (heading) {
      return /^agences (detectees|détectées|locales)/i.test((heading.textContent || "").trim());
  ...
  function enforceContentOrder() {
    const map = document.getElementById("map");
    const listings = document.getElementById("allListingsSection");
    if (!map || !listings || !map.parentNode) {
      return listings;
    }
    map.parentNode.insertBefore(listings, map.nextSibling);
    let cursor = listings;
    agencyContactBlocks().forEach(function (block) {
  ...
  function renderOtherSources(payload, config) {
    ...
    const agencyTail = enforceContentOrder();
    if (agencyTail && agencyTail.parentNode) {
      agencyTail.parentNode.insertBefore(section, agencyTail.nextSibling);
    } else if (listingsSection) {
      listingsSection.parentNode.insertBefore(section, listingsSection.nextSibling);
    } else {
      anchor.parentNode.insertBefore(section, anchor);
    }
    enhanceResponsiveTables(section);
  }
  ...
  function renderUnifiedListings(payload, config) {
    ...
    if (map && map.parentNode) {
      map.parentNode.insertBefore(section, map.nextSibling);
  ...
  async function notifyNewListings(newListings) {
    ...
    const profile = activeSearchProfile();
    const title = newListings.length === 1 ? "1 nouveau bien — " + profile.label : newListings.length + " nouveaux biens — " + profile.label;
  ...
  async function checkForNewListings(manual) {
    try {
      ...
      const listings = Array.isArray(payload.listings) ? payload.listings : [];
      const initialized = localStorage.getItem(modeStorageKey(INIT_KEY)) === "1";
