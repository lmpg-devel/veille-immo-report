...
.map-popup-actions button { border: 0; background: #fff; color: #0b5c86; font: 600 12px Arial, sans-serif; padding: 0; cursor: pointer; }
...
function staticMapPopupHtml(item) {
  const contact = [item.agent, item.phone].filter(Boolean).map(escapeHtml).join(" · ") || "non publie";
  return [
    "<div class='map-popup'>",
    "<div class='map-popup-source'>" + escapeHtml(item.source || "Source") + "</div>",
    "<div class='map-popup-title'>" + escapeHtml(item.title || "Annonce") + "</div>",
    "<div class='map-popup-price'>" + escapeHtml(money(item.price)) + "</div>",
    "<div class='map-popup-details'><span><strong>Precision</strong> " + escapeHtml(item.precision || "non publiee") + "</span></div>",
    item.address ? "<div class='map-popup-address'>" + escapeHtml(item.address) + "</div>" : "",
    "<div class='map-popup-contact'><strong>Contact</strong> " + contact + "</div>",
    "<div class='map-popup-actions'><button type='button' class='external-link-button' data-external-url='" + escapeHtml(item.url) + "'>Ouvrir l'annonce</button></div>",
    "</div>"
  ].join("");
}
...
if (window.L && (listingMarkers.length || agencyMarkers.length)) {
  ...
  listingMarkers.forEach((item) => {
    ...
    marker.bindPopup(staticMapPopupHtml(item));
...
foreach ($location in $config.locations) {
  ...
  foreach ($listing in $listings) {
    if (-not ($allListings | Where-Object { $_.Source -eq $listing.Source -and $_.Id -eq $listing.Id })) {
...
$html = New-HtmlReport -Config $config -Listings $listingsArray -PortalLinks $portalLinksArray -LocalAgencies $localAgenciesArray -RunAt $runAt
